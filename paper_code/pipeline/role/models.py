...
#         spans_embedding = torch.cat((spans_start_embedding, spans_end_embedding, spans_width_embedding), dim=-1)
    ...
    def forward(self, input_ids, spans, spans_mask, spans_role_label=None, token_type_ids=None, attention_mask=None):
        ...
        if spans_role_label is not None:
            loss_fct = CrossEntropyLoss(reduction='sum')
            if attention_mask is not None:
                active_loss = spans_mask.view(-1) == 1
                active_logits = logits.view(-1, logits.shape[-1])
                active_labels = torch.where(
                    active_loss, spans_role_label.view(-1), torch.tensor(loss_fct.ignore_index).type_as(spans_role_label)
                )
                loss = loss_fct(active_logits, active_labels)
            else:
                loss = loss_fct(logits.view(-1, logits.shape[-1]), spans_role_label.view(-1))
...
class RoleModel():
    ...
    def __init__(self, model_path, num_role_labels):
        super().__init__()

        logger.info('Loading BERT model from {}'.format(model_path))

        self.tokenizer = BertTokenizer.from_pretrained(model_path)
        self.bert_model = BertForRole.from_pretrained(model_path, num_role_labels=num_role_labels, max_span_length=8, return_dict=False)
    ...
    def _get_input_tensors(self, tokens, spans, spans_role_label):
        start2idx = []
        end2idx = []
        ...
        return tokens_tensor, bert_spans_tensor, spans_role_label_tensor

    def _get_input_tensors_batch(self, samples_list, use_gold=True):
        tokens_tensor_list = []
        ...
        for tokens_tensor, bert_spans_tensor, spans_role_label_tensor in zip(tokens_tensor_list, bert_spans_tensor_list, spans_role_label_tensor_list):
            # padding for tokens
...
#                 logger.info('bert_spans_tensor: %s'%(bert_spans_tensor,))
        ...
        return final_tokens_tensor, final_attention_mask, final_bert_spans_tensor, final_spans_mask_tensor, final_spans_role_label_tensor, sentence_length

    def run_batch(self, samples_list, use_gold=True, training=True):
        # convert samples to input tensors
        tokens_tensor, attention_mask_tensor, bert_spans_tensor, spans_mask_tensor, spans_role_label_tensor, sentence_length = self._get_input_tensors_batch(samples_list, use_gold)
        ...
        if training:
            ...
            role_loss, role_logits, spans_embedding = self.bert_model(
            ...
            output_dict['role_loss'] = role_loss.sum()
            output_dict['role_llh'] = F.log_softmax(role_logits, dim=-1)
        else:
            self.bert_model.eval()
            with torch.no_grad():
                role_logits, spans_embedding, last_hidden = self.bert_model(
                    input_ids = tokens_tensor.to(self._model_device),
                    spans = bert_spans_tensor.to(self._model_device),
                    spans_mask = spans_mask_tensor.to(self._model_device),
            ...
            for i, sample in enumerate(samples_list):
                ...
                prob = []
                lh = []
                for j in range(len(sample['pred_spans'])):
                    role.append(predicted_label[i][j])
                    # prob.append(F.softmax(role_logits[i][j], dim=-1).cpu().numpy())
                    prob.append(role_logits[i][j].cpu().numpy())
                    lh.append(last_hidden[i][j])
                predicted.append(role)
                pred_prob.append(prob)
                hidden.append(lh)
            output_dict['pred_role'] = predicted
            output_dict['role_probs'] = pred_prob
            output_dict['role_last_hidden'] = hidden
